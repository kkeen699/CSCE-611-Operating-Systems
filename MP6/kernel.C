...
void fun2() {
    ...
    for(int j = 0;; j++) {
       ...
       /* -- Read */
       Console::puts("Thread2 Reading a block from disk...\n");
       SYSTEM_DISK->read(read_block, buf);

       /* -- Display */
       Console::puts("Thread2");
...
void fun3() {
    ...
    int  write_block = 0;
     ...
     for(int j = 0;; j++) {
...
#else
        /* -- Read */
       Console::puts("Thread3 Reading a block from disk...\n");
       SYSTEM_DISK->read(read_block, buf);
       ...
       SYSTEM_DISK->write(write_block, buf); 
       Console::puts("Thread3 write done\n");
       /* -- Move to next block */
       write_block = read_block;
       read_block  = (read_block + 1) % 10;
       Console::puts("Thread3 finish\n");
#endif
       pass_on_CPU(thread4);
    }
}
